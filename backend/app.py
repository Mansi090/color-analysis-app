...
def upload_image():
    logger.info("Accessing upload route")
    if request.method == 'POST':
        logger.info("Received POST request for upload")
        if 'image' not in request.files:
            logger.error("No image in POST request")
    ...
    try:
        ...
        return jsonify({"error": "Yo, upload.html is missing in templates folder! 😿 Create it and try again."}), 500
